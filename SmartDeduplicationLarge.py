...
restaurant_db_cols = ['LineNumber', 'CompanyName', 'CIKKey']
restaurant_db = pd.read_csv(r'Companies.csv', sep=',', names=restaurant_db_cols, encoding="latin-1")
pd.set_option("display.max_rows", None, "display.max_columns", None)

restaurant_db['LineNumber'] = restaurant_db['LineNumber'].map(lambda x: re.sub(r'\W+', '', str(x)))
restaurant_db['CompanyName'] = restaurant_db['CompanyName'].map(lambda x: re.sub(r'\W+', '', str(x)))
restaurant_db['CIKKey'] = restaurant_db['CIKKey'].map(lambda x: re.sub(r'\W+', '', str(x)))
...
def create_hash_column(restaurant_db_array_in,df_name):
    ...
    for row in restaurant_db_array_in:
        ...
        for column in row:
            ...
            if count == 1:
                line_hash.append(column_hash)
            elif count == 2:
                name_hash.append(column_hash)
            elif count == 3:
                cik_hash.append(column_hash)

            count+=1
        data_hash.append(total_sum_column)
    add_hash_to_df(line_hash,name_hash,cik_hash,data_hash,df_name)


def add_hash_to_df(line_hash,name_hash,cik_hash,data_hash,df_name):
    if df_name == 0:
        restaurant_db['LineHash'] = line_hash
        restaurant_db['NameHash'] = name_hash
        restaurant_db['CIKHash'] = cik_hash
        #restaurant_db['StyleHash'] = style_hash
        restaurant_db['DataHash'] = data_hash
    else:
        new_restaurant['LineHash'] =  line_hash
        new_restaurant['NameHash'] =name_hash
        new_restaurant['CIKHash'] = cik_hash
        # restaurant_db['StyleHash'] = style_hash
...
def find_duplicate_percentage(restaurant_db_array_cluster_wise):
    duplicate_phone = restaurant_db_array_cluster_wise[restaurant_db_array_cluster_wise.duplicated(['CIKHash'],keep=False)]
    duplicate_phone = duplicate_phone.sort_values(by=['CIKHash'])
    if duplicate_phone.empty:
        return 0,None
    else:

        duplicate_address = duplicate_phone[duplicate_phone.duplicated(['NameHash'],keep = False)]
        duplicate_address = duplicate_address.sort_values(by=['NameHash'])
...
def plot_data():
    fig, ax = plt.subplots()
    ax.scatter(restaurant_db['NameHash'], restaurant_db['CIKHash'])
...
restaurant_db_graph = restaurant_db[['NameHash', 'CIKHash']].copy() #'NameHash', 'AddressHash', 'PhoneHash', 'StyleHash','DataHash'
...
#Uncomment this section:
        ...
        for i in range(2):
            ...
            for cluster in np.unique(predict_label):
                ...
                array_frame_cluster[cluster] = pd.DataFrame(restaurant_db_array_test[(find_cluster_indices(cluster, predict_label))],columns=['LineNumber', 'CompanyName', 'CIKKey','LineHash', 'NameHash','CIKHash', 'DataHash'])
            ...
            #if len(total_duplicate_df_obj) != 0:
...
def single_duplicate_checker(predict_label,labels):
    ...
    array_frame_cluster = pd.DataFrame(restaurant_db_array[(find_cluster_indices(predict_label, labels))],
                                       columns=['LineNumber', 'CompanyName', 'CIKKey','LineHash', 'NameHash', 'CIKHash', 'DataHash'])
    duplicate_phone = array_frame_cluster[array_frame_cluster.duplicated(['CIKHash'], keep=False)]

    check_phone = new_restaurant['CIKHash']
    df = duplicate_phone.where(duplicate_phone.values == new_restaurant.values)

    df = df.dropna(thresh=1)
    df = df.drop(columns=['NameHash', 'CIKHash', 'DataHash'])

    output = duplicate_phone.loc[df.index]
    if len(output)>0:
        print("The records which have at least 50% of match are: \n",output.drop(columns=['NameHash', 'CIKHash', 'DataHash']))
    else:
...
LineNumber = str(len(restaurant_db_graph_normalised.index))
Name = input("Enter the Name of the Company: ") #'Arts Deli'#
CIKKey = input("Enter the CIK Key of the company : ") #'12224 Ventura Blvd. Studio City'#

new_restaurant = pd.DataFrame({"LineNumber":[LineNumber], "CompanyName": [Name], "CIKKey":[CIKKey]})
new_restaurant['LineNumber'] = new_restaurant['LineNumber'].map(lambda x: re.sub(r'\W+', '', x))
new_restaurant['CompanyName'] = new_restaurant['CompanyName'].map(lambda x: re.sub(r'\W+', '', str(x)))
new_restaurant['CIKKey'] = new_restaurant['CIKKey'].map(lambda x: re.sub(r'\W+', '', str(x)))
#new_restaurant['Style'] = new_restaurant['Style'].map(lambda x: re.sub(r'\W+', '', str(x)))

new_restaurant_db_array = convert_frame_to_array(new_restaurant)
create_hash_column(new_restaurant_db_array,1)
new_restaurant_db_array = convert_frame_to_array(new_restaurant)
#print(restaurant_db_array.head(4))
print(new_restaurant_db_array)
restaurant_db_array = np.vstack((restaurant_db_array,new_restaurant_db_array))
#print(new_restaurant)
new_restaurant_hash = new_restaurant[['NameHash','CIKHash']].copy() #,'PhoneHash','StyleHash''NameHash','AddressHash','PhoneHash','StyleHash','DataHash'
